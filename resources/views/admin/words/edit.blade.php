    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                ✏️ {{ __('Edit Word:') }} <span class="text-indigo-600">{{ $word->word }}</span>
            </h2>
        </div>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('admin.words.update', $word) }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="phonetic_spelling" id="phonetic_spelling" value="{{ old('phonetic_spelling', $word->phonetic_spelling) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="phonemes" id="phonemes" value="{{ old('phonemes', $word->phonemes ? implode(', ', $word->phonemes) : '') }}"
                               class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                    ...
                    <div>
                        ...
                        <textarea name="example_sentence" id="example_sentence" rows="3"
                                  class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                  placeholder="e.g., The cat sat on the mat.">{{ old('example_sentence', $word->example_sentence) }}</textarea>
