    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('admin.phrases.update', $phrase) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')

                    <div>
                        <label for="phrase" class="block text-sm font-medium text-gray-700 mb-2">Phrase *</label>
                        <input type="text" name="phrase" id="phrase" value="{{ old('phrase', $phrase->phrase) }}" required
                    ...
                    <div>
                        ...
                        <textarea name="meaning" id="meaning" rows="2"
                                  ...
                                  placeholder="e.g., A greeting used in the morning">{{ old('meaning', $phrase->meaning) }}</textarea>
                        @error('meaning')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="example_sentence" id="example_sentence" rows="2"
                                  class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                  placeholder="e.g., Good morning! How are you today?">{{ old('example_sentence', $phrase->example_sentence) }}</textarea>
                    ...
                    <div>
                        ...
                        <select name="difficulty" id="difficulty" required
                                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            @for($i = 1; $i <= 10; $i++)
                                <option value="{{ $i }}" {{ old('difficulty', $phrase->difficulty) == $i ? 'selected' : '' }}>
